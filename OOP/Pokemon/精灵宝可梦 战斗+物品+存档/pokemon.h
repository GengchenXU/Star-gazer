...
public:
	...
	friend Pokemon* info_load(ifstream& filestream);
	friend void info_save(ofstream& filestream, Pokemon* pokemon);
};
#pragma once
...
public:
	Pokemon* user;	//使用者
	Item(int num, Pokemon* u) { number = num; user = u; ID = 0; }
...
{
public:
	HPpotion(int num, Pokemon* u) :Item(num, u) { SetName("HP药"); SetID(1); }
	int function()
	{
...
{
public:
	SPpotion(int num, Pokemon* u) :Item(num, u) { SetName("SP药"); SetID(2);  }
	int function()
	{
...
{
public:
	Bomb(int num, Pokemon* u) :Item(num, u) { SetName("炸药"); SetID(3);}
